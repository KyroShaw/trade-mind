...
**验收标准**：
- [x] Cron 任务成功写入板块和消息数据（实现完毕，需真实 API key 运行时验证）
- [x] `getSectors` 返回 ≥ 5 个板块，每板块 ≥ 1 龙头
- [x] `getNews` 返回 ≥ 10 条消息，每条含 aiSummary
- [x] 7 天历史数据自动清理
- [x] 外部 API 失败时使用上次缓存数据（不中断服务）
...
**验收标准**：
- [x] 板块 Tab 展示 ≥ 5 个板块，可展开龙头
- [x] 消息 Tab 展示 ≥ 10 条消息，支持标签筛选
- [x] 数据更新时间标注正确
- [x] 未登录可查看列表（AI 摘要可见）
